...
23 merged PRs, 143 passing tests, 6 SQLite migrations. Two adapters are real daemon-driven producers today: `ClaudeCodeTailerAdapter` (watchdog on `~/.claude/transcripts/*.jsonl`) and `FileWatcherAdapter` (inbox archives). Eight more are scaffolded as `_AdapterBase` subclasses but only get started when they move into `ACTIVE_PRODUCERS`. The browser extension in `pke-ext/` runs standalone; the HTTP proxy adapters need streaming pass-through before they're useful.

个人项目，自己用着合适为主。bug 可以提 issue。我不承诺长期维护，但东西坏了我大概率想知道。
