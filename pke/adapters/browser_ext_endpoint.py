...
def _coerce_source(payload: dict[str, Any], url: str) -> str:
    ...
    if (
        ...
        and requested.startswith("browser_ext")
        and requested in VALID_SOURCES
    ):
        return requested
    # Map URL-derived guesses for forward compatibility — used when an
    # older content script omits ``source`` or sends a value we haven't
